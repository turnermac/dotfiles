...
local function tile_group(cls, wa, orientation, fact, group)
    ...
    for c = group.first,group.last do
		...
		local border_gap = tonumber(beautiful.border_width * 2)
        ...
        then
            ...
            if top then
                geom[height] = geom[height] - 2 * useless_gap - border_gap
                geom[y] = geom[y] + useless_gap
            else
                geom[height] = geom[height] - useless_gap - border_gap
            end

            if left then
                geom[width] = geom[width] - 2 * useless_gap - border_gap
                geom[x] = geom[x] + useless_gap
            else
                geom[width] = geom[width] - useless_gap - border_gap
            end
        end
        -- End of useless gap.
